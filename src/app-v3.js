...
class ExpressionTrainer {
      ...
      if (analysis.suggestions && analysis.suggestions.length > 0) {
        analysis.suggestions.forEach(s => {
          if (s.type === 'vague' && s.alternatives) {
            this.addFeedbackItem(`「${s.original}」→ ${s.alternatives.join(' / ')}`, 'vague');
          } else if (s.type === 'filler') {
            this.addFeedbackItem(s.message, 'filler');
          } else if (s.type === 'hedge') {
            this.addFeedbackItem(s.message, 'hedge');
          } else {
            this.addFeedbackItem(s.message, 'ai');
          }
        });
      }
    }
  ...
  async requestRealtimeFeedback() {
    ...
    if (result.success && result.feedback) {
      this.addFeedbackItem(result.feedback, 'ai');
    }
  }
