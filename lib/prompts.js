 ...
 */
function getReportPrompt(fullText, stats, customPrompt, conversation = '') {
  const conversationBlock = conversation ? `\n\n以下是完整对话记录。请单独分析说话者部分，并引用对话上下文，但不要把 AI 内容计入说话者的语速、填充词或表达密度：\n---\n${conversation}\n---` : '';
...
报告开头第一句话固定为：「超级表达训练营收到你的录音啦~~」（如果输入是逐字稿则改为「超级表达训练营收到你的逐字稿啦~~」），然后空一行再开始正文。
...
---
    ...
    数据:${stats.duration}秒 | ${stats.totalWords}字 | 填充词${stats.fillers}次 | 犹豫词${stats.hedges}次 | 笼统词${stats.vagueWords}次${conversationBlock}`
  };
