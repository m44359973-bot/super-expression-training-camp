 ...
 * @returns {string} 报告文本
 */
async function sendReport(fullText, stats, settings, customPrompt, conversation = '') {
  const config = getProviderConfig(settings);
  const prompt = getReportPrompt(fullText, stats, customPrompt, conversation);
...
/**
 * 发送一轮对话消息。沿用设置页中的 OpenAI 兼容后端配置。
 */
async function sendDialogue(messages, settings, customPrompt) {
  const config = getProviderConfig(settings);
  const system = `你是中文口语练习中的对话伙伴。请自然、简洁地回应对方，并追问一个具体问题推动对话继续。不要评价口语好坏，不要输出分析报告。每次回复控制在80字以内。${customPrompt?.styleRef ? `\n对话风格：${customPrompt.styleRef}` : ''}`;
  const dialogueMessages = [
    { role: 'system', content: system },
    ...messages.slice(-12)
  ];
  return callAPI(config.endpoint, config.apiKey, config.model, dialogueMessages, 300);
}
...
module.exports = { sendFeedback, sendReport, sendDialogue, testConnection };
